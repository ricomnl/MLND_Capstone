...
def convert(annotationsPathBoxes, annotationPathBulbs):
    label_dict = {"go": 0, "goforward":1, "goleft":2, "warning":3, "warningleft":4, "stop":5, "stopleft":6}
    box = pd.read_csv(annotationsPathBoxes, delimiter=';')
    bulbs = pd.read_csv(annotationPathBulbs, delimiter=';')
    
    # Concat them, sort values by filename, drop unneccessary columns and reset index
    boxes = pd.concat([box, bulbs]).sort_values(by=['Filename']).drop(['Origin file', 
                      'Origin frame number', 'Origin track', 'Origin track frame number'], axis=1)
    boxes = boxes.reset_index(drop=True)
    
    # Convert the class by using label_dict and process filename
    for i, row in boxes.iterrows():
        boxes.loc[i, 'Annotation tag'] = label_dict[row['Annotation tag'].lower().lstrip()]
        if 'day' in boxes.loc[i, 'Filename']:
            daytime = 'dayTraining/'
        else:
            daytime = 'nightTraining/'
        boxes.loc[i, 'Filename'] = row['Filename'].replace(daytime, '')
